...
INPUT_FOLDER="/var/lib/transmission-daemon/downloads/South Park Season 5  (1080p x265 BD 10bit Joy)"  # Chemin vers le dossier contenant les fichiers vidéo
...
while true; do
    ...
    ffmpeg -re -stream_loop -1 -f concat -safe 0 -i "$PLAYLIST_FILE" \
        -c:v libx264 -preset ultrafast -b:v 1500k -maxrate 2000k -bufsize 5000k \
